import { cuserData } from '@/api/newDataStatic'
export default {
  data() {
    return {
      topData: {
  ...
  methods: {
    ...
    getList() {
      cuserData(this.formData).then(res => {
        const { allNomUser, todayNewUser, userVisit, count } = res.data
        this.topData = { allNomUser, todayNewUser, userVisit, count }
